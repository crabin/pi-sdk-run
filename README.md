...
- `sales`：基于 `data/sales.csv` 的销售数据分析助手；
...
`agentId` 可省略，默认使用 `reach`。非法请求返回 `400`，同一 Agent 同时执行多个请求返回 `429`，服务关闭期间返回 `503`。当前示例是本地单进程服务，不提供用户级隔离、跨进程会话持久化或多租户安全边界。
...
- `src/agent/runner.ts`：唯一的 SDK 会话适配和生命周期实现；
